...
Total_Emmisssions <- aggregate(Baltimore$Emissions, by=list(Baltimore$year), FUN=sum)
...
with(Total_Emmisssions,plot(Year,Tot_Emssn, ylab = "Total Annual Emission", pch=20,cex=2,col="blue", main="Yearly Trend of Pm2.5 in Baltimore", xaxt='n'))
axis(side = 1, at=Total_Emmisssions$Year)
lin_model <- with(Total_Emmisssions,lm(Tot_Emssn~Year))
abline(lin_model, col="green", lwd=2)
